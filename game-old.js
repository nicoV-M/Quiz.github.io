...
import { quizCuisine } from './questions.js';
...
const totalQuestions = quizChoisi.questions.length; // Nombre total de questions

function choixDuQuiz() {
  const boutonCuisine = document.querySelector('#choixCuisine');
  const boutonVoyage = document.querySelector('#choixVoyage');
  contenuQuestion.innerText = 'Choisis ton thème de quiz'
  boutonCuisine.addEventListener('click', () => {
    quizChoisi = quizCuisine;
    boutonCuisine.style.display = 'none';
    boutonVoyage.style.display = 'none';
    loadQuestion();
    timer();
    console.log("clicked")
    })

  boutonVoyage.addEventListener('click', () => {
    quizChoisi = quizVoyage
    boutonCuisine.style.display = 'none';
    boutonVoyage.style.display = 'none';
    loadQuestion();
    timer();
...
function loadQuestion() {
  ...
  const questionActuelle = quizChoisi.questions[i];
...
boutonSuivant.addEventListener('click', () => {
  ...
  if (i < quizChoisi.questions.length) {
...
function checkAnswer() {
  ...
  buttons.forEach(button => {
    button.addEventListener('click', () => {
      ...
      console.log("score", score, "sur", quizChoisi.questions.length);
...
function commentaireScore(score, meilleurScore) {
  ...
  if (score === 0) {
    ...
    text = 'Incroyable ! Tu es incollable sur le sujet 🎊!';
      ...
      return `${text}<br><br>🎯 Dernier score : ${dernierScore} / ${quizChoisi.questions.length} 🎯<br>🏆 Meilleur score : ${meilleurScore} / ${quizChoisi.questions.length} 🏆`;
}
...
console.log(score, "sur", quizChoisi.questions.length);
